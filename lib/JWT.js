...
if (isNode) {
	...
	if (typeof SHA256_init === 'undefined') {
		throw new Error('This JWT library requires the JSSHA256 Library if used in the browser.');
	}
	if (typeof window.btoa === 'undefined') {
		throw new Error('Your browser has not built in support for base64 encoding. Use a WebKIT for now...');
	}
	if (typeof jQuery === 'undefined') {
		throw new Error('This JWT Library requires JQuery to be loaded, if used in browser.');
	}
...
jwt = function(useroptions, additionalReservedClaims) {
	...
	function validate (jwt) {
		...
		if (!decomposedSegments instanceof Array) throw new Error('Error when trying to decompose the JWT Segments.');
		if (decomposedSegments.length !== 3) throw new Error('Wrong number of segments in JWT, separated by "." Did expect 3.');
		...
		if (!header['alg']) throw new Error('Encoded JWT did not include the [alg] header property, which is required for validating the signature.');

		digest = createSignature(cryptoInput, header['alg']);

	//	if (digest !== cryptoSegment) throw new Error('Invalid HMAC Signature in CryptoSegment of the provided JWT.');
	...
	function requireClaim (claimname) {
		if (!claims.hasOwnProperty(claimname)) {
			throw new Error('JWT is missing required Claim [' + claimname + ']');
		}
	}
	...
	function validateClaims () {
		...
		if (claims['exp']) {
			if (claims['exp'] < (currentTime - SKEW)) {
				throw new Error('JWT is expired, based upon the [exp] header.');
			}
		}

		if (claims['nbf']) {
			if (claims['nbf'] > (currentTime + SKEW)) {
				throw new Error('JWT is not yet valid, based upon the [nbf] header.');
			}
		}
		...
		if (claims['iss']) {
			if (options['iss']) {
				if (claims['iss'] !== options['iss']) {
					throw new Error('JWT had an invalid Issuer');
				}
			}
		}
		...
		if (claims['typ']) {
			if (claims['typ'] !== 'JWT') {
				throw new Error('The JWT type was invalid. Only type JWT is supported.');
			}
		}
		...
		for (key in claims) {
			if (claims.hasOwnProperty(key)) {
				...
				if (!acceptedClaims[key]) {
					throw new Error('Found unsupported / unreckognized claim in JWT: ' + key);
				}
			}
		}
	...
	function createSignature (cryptoInput, alg) {
		...
		if (alg !== 'HS256') throw new Error('Algoritm not implemented. Currently only HS256 is implemented.');

		if (!options['secret']) throw new Error('Missing [secret] option to perform HS256 alg signature.');
	...
	function sign () {
		...
		if (!options['alg']) throw new Error('Missing [alg] option in options.');
		if (options['alg'] !== 'HS256') throw new Error('Configured alg is not implemented.');
	...
	function init (content) {
		...
		if (typeof content === 'undefined') {
			...
			throw new Error('Initialization of a JWT object must be done with an javascript object, or a packed string, or undefined.');
		}
		return my;
	}
